	...
	*/
	...
	var Connection = class extends _eventemitter.default {
		...
		buildURI(protocol) {
			...
			if (protocol === "ws") {
				uri += "/stream";
				const params = [];
				if (deltaStreamBehaviour && SUPPORTED_STREAM_BEHAVIOUR.hasOwnProperty(deltaStreamBehaviour) && SUPPORTED_STREAM_BEHAVIOUR[deltaStreamBehaviour] !== "") params.push(`subscribe=${SUPPORTED_STREAM_BEHAVIOUR[deltaStreamBehaviour]}`);
				if (sendMeta && SUPPORTED_SEND_META.hasOwnProperty(sendMeta) && SUPPORTED_SEND_META[sendMeta] !== "") params.push(`sendMeta=${SUPPORTED_SEND_META[sendMeta]}`);
		...
		reconnect(initial = false) {
			...
			return this.fetch("/auth/login", authRequest).then((result) => {
				if (!result || typeof result !== "object" || !result.hasOwnProperty("token")) throw new Error(`Unexpected response from auth endpoint: ${JSON.stringify(result)}`);
		...
		initiateSocket() {
			if (isNode && this.options.useTLS && this.options.rejectUnauthorized === false) this.socket = new _isomorphicWs.default(this.wsURI, { rejectUnauthorized: false });
		...
		sendKeepaliveWithReschedule() {
			if (this.connected === true) {
				if (this.lastMessage < Date.now() - this.wsKeepaliveIntervalMs) this.socket.send("{}");
		...
		_onWSMessage(evt) {
			...
			if (data && typeof data === "object" && data.hasOwnProperty("name") && data.hasOwnProperty("version") && data.hasOwnProperty("roles")) this.connectionInfo = data;
		...
		subscribe(subscriptions = []) {
			if (!Array.isArray(subscriptions) && subscriptions && typeof subscriptions === "object" && subscriptions.hasOwnProperty("subscribe")) subscriptions = [subscriptions];
			subscriptions.forEach((sub) => {
				this.send(JSON.stringify(sub));
			});
		}
		send(data) {
			if (this.connected !== true || this.socket === null) return Promise.reject(/* @__PURE__ */ new Error("Not connected to WebSocket"));
		...
		fetch(path, opts) {
			...
			if (isNode && this.options.useTLS && this.options.rejectUnauthorized === false) opts.agent = new _https.default.Agent({ rejectUnauthorized: false });
			let URI = `${this.httpURI}${path}`;
			if (URI.includes("/api/auth/login")) URI = URI.replace("/api/auth/login", "/auth/login");
			if (URI.includes("/api/access/requests")) URI = URI.replace("/api/access/requests", "/access/requests");
			if (URI.includes("/signalk/v1/api/security")) URI = URI.replace("/signalk/v1/api/security", "/security");
			debug(`[fetch] ${opts.method || "GET"} ${URI} ${JSON.stringify(opts, null, 2)}`);
			return (0, _crossFetch.default)(URI, opts).then((response) => {
				if (!response.ok) throw new Error(`Error fetching ${URI}: ${response.status} ${response.statusText}`);
	...
	var flattenSubscriptions = (subscriptionCommands) => {
		...
		subscriptionCommands.forEach((command) => {
			if (!Array.isArray(commandPerContext[command.context])) commandPerContext[command.context] = [];
	...
	var Request = class extends _eventemitter.default {
		constructor(connection, name, body) {
			...
			this.connection.on("message", (message) => {
				if (message && typeof message === "object" && message.hasOwnProperty("requestId") && message.requestId === this.requestId) this.addResponse(message);
			});
		}
		query() {
			const request = {
...
var require_client$1 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	*/
	...
	var AUTHENTICATION_REQUEST = exports.AUTHENTICATION_REQUEST = "__AUTHENTICATION_REQUEST__";
	...
	var Client = class extends _eventemitter.default {
		constructor(options = {}) {
			...
			if (Array.isArray(this.options.subscriptions)) this.subscribeCommands = this.options.subscriptions.filter((command) => isValidSubscribeCommand(command));
			if (this.options.notifications === true) this.subscribeCommands.push({
			...
			if (this.options.autoConnect === true) this.connect().catch((err) => this.emit("error", err));
		}
		get self() {
		...
		authenticate(username, password) {
			const request = this.request(AUTHENTICATION_REQUEST, { login: {
				username,
				password
			} });
			request.on("response", (response) => {
				...
				this.emit("error", /* @__PURE__ */ new Error(`Error authenticating: status ${response.statusCode}`));
			});
			request.send();
		}
		request(name, body = {}) {
		...
		subscribe(subscriptions = []) {
			...
			if (subscriptions && !Array.isArray(subscriptions) && typeof subscriptions === "object" && subscriptions.hasOwnProperty("subscribe")) subscriptions = [subscriptions];
		...
		unsubscribe() {
			...
			if (this.subscribeCommands.length > 0) this.connection.subscribe(this.subscribeCommands);
		}
		connect() {
		...
		API() {
			if (this.connection === null) return Promise.reject(/* @__PURE__ */ new Error("There are no available connections. Please connect before you use the REST API."));
			if (this.api !== null) return Promise.resolve(this.api);
			return new Promise((resolve) => {
				this.api = new _api.default(this.connection);
				if (this.fetchReady === true || this.options.useAuthentication === false) return resolve(this.api);
				this.connection.on("fetchReady", () => {
					resolve(this.api);
				});
			});
		...
		checkAndEmitNotificationsInDelta(delta) {
			if (this.options.notifications === false || !delta || typeof delta !== "object" || !Array.isArray(delta.updates)) return;
			const notifications = {};
			delta.updates.forEach((update) => {
				(update.values || []).forEach((mut) => {
					if (typeof mut.path === "string" && mut.path.includes("notifications.")) notifications[mut.path.replace("notifications.", "")] = { ...mut.value };
				});
			});
	...
	var flattenTree = (tree) => {
		...
		const evaluateLeaf = (key) => {
			...
			if (cursor && typeof cursor === "object" && cursor.hasOwnProperty("value")) flattened[currentPath] = Object.assign({}, cursor.value);
	...
	var isValidSubscribeCommand = (command) => {
		...
		if (!command.hasOwnProperty("context") || !Array.isArray(command.subscribe)) return false;
		return true;
	};
}));
//#endregion
	...
	*/
	...
	var Discovery = class extends _eventemitter.default {
		constructor(bonjourOrMdns, timeout = 6e4) {
			super();
			this.found = [];
			if (!bonjourOrMdns || typeof bonjourOrMdns !== "object") throw new Error("No mDNS provider given");
			...
			if (Object.keys(bonjourOrMdns).join(",").startsWith(bonjourProps)) return this.discoverWithBonjour(bonjourOrMdns, timeout);
			if (Object.keys(bonjourOrMdns).join(",").startsWith(mdnsProps)) return this.discoverWithMdns(bonjourOrMdns, timeout);
		...
		handleDiscoveredService(ad, service) {
			if (typeof service.roles === "string") service.roles = service.roles.split(",").map((role) => role.trim().toLowerCase());
			service.roles = Array.isArray(service.roles) ? service.roles : [];
			let ipv4 = service.hostname;
			if (Array.isArray(ad.addresses)) ipv4 = ad.addresses.reduce((found, address) => {
				if (address && typeof address === "string" && address.includes(".")) found = address;
				return found;
			}, service.hostname);
...
var InfoPanel = L.Control.extend({
	...
	setStatus: function(notification) {
		if (notification) {
			...
			if (notification.value.message !== "Off" && notification.value.state) this._pluginStatus.classList.add(notification.value.state);
...
var WindBarbViewBox = Object.freeze(Object.fromEntries(Object.entries(WindBarb).map(([key, path]) => {
	const minY = computeMinY(path);
	const top = Math.floor(minY - VIEWBOX_TOP_PADDING);
	return [key, `${VIEWBOX_X} ${top} ${VIEWBOX_WIDTH} ${VIEWBOX_BOTTOM - top}`];
})));
...
var hasNestedProperty = (obj, prop, ...rest) => {
	...
	if (rest.length === 0 && Object.prototype.hasOwnProperty.call(obj, prop)) return true;
...
var AnchorOverlay = class {
	...
	update(state) {
		this.setBoatPosition(state.getPosition(), state.boatConfig.heading, state.boatConfig.gpsOffset);
	}
	_refreshLine() {
		if (!this.boatPosition) return;
		const bow = GeoMath.calculateBowCoordinates(this.boatPosition, this.heading, this.gpsOffsets.x, this.gpsOffsets.y);
...
var AnchorController = class {
	...
	estimateAnchorPosition() {
		...
		const guess = GeoMath.calculateDestinationPoint(bow.lat, bow.lng, this._appState.boatConfig.heading, distance);
...
(class AnchorAlarm {
	...
	handleDeltas(delta) {
		if (delta.updates) {
			for (const update of delta.updates) if (update.values) {
				let timestamp = update.timestamp;
				for (const value of update.values) this.state.handleDelta(timestamp, value);
			}
		}
	}
	init() {
