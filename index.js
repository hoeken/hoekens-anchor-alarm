...
module.exports = function (app) {
  ...
  let requiredPaths = [
    ...
    },
    {
      path: "design.draft",
      description:
        "Optional - used for depth calculations. Edit Server -> Settings",
    },
    {
      path: "design.aisShipType",
      description:
        "Optional - used to chose the correct icon. Edit Server -> Settings",
    },
    {
      path: "environment.depth.belowTransducer",
      description:
        "Optional - used for scope calculations. No depthsounder found.",
    },
    {
      path: "environment.depth.transducerToKeel",
      description:
        "Optional - used for scope calculations. You can set it with configuration options below. ",
    },
    {
    ...
    schema = {
      ...
      properties: {
        ...
        transducerToKeel: {
          type: "number",
          title:
            "Distance between the transducer and the keel (in meters, should be negative).  Useful if your install doesn't automatically detect this. 0 and above are ignored.",
          default: 0,
        },
        on: {
          type: "boolean",
  ...
  plugin.start = function (props) {
    ...
    try {
      ...
      if (
        typeof configuration["transducerToKeel"] != "undefined" &&
        parseFloat(configuration["transducerToKeel"]) < 0
      ) {
        app.handleMessage(plugin.id, {
          updates: [
            {
  ...
  function checkPosition(app, plugin, vesselPosition, configuration) {
    ...
    try {
      const chordStart =
        configuration.segmentOriention + configuration.segmentSize / 2 || 0;
      const chordEnd =
        configuration.segmentOriention - configuration.segmentSize / 2 + 360 ||
        360;
      const arc = turf.lineArc(
        [anchorPosition.longitude, anchorPosition.latitude],
        maxRadius,
        chordStart,
        chordEnd,
        { units: "meters" },
      ...
      vesselIsInAnchorAreaPolygon = turf.booleanPointInPolygon(
        turf.point([vesselPosition.longitude, vesselPosition.latitude]),
        anchorAreaPolygon,
      );
